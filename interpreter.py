...
class Interpreter(object):
    ...
    def do_layer(self, layer):
        for operator in layer.operators:
            if operator['type'] == 'DEFAULT':
                ...
                if not self.do_for_operator(operator['head'], operator['sublayer']):
                    break
    ...
    def do_default_operator(self, operator, layer):
        ...
        if triade['type'] == 'function':
            ...
            if not layer.set_variable(name, value, iter):
                print("Ошибка: Несовпадение размеров массивов [Строка:{}]".format(operator[0][2]+1))
                return False
            ...
            if valueF is None:
                ...
                return False
            if not layer.set_variable(name, valueF-valueS, iter):
                print("Ошибка: Несовпадение размеров массивов [Строка:{}]".format(operator[0][2]+1))
                return False
            ...
            if valueF is None:
                print("Ошибка: использование неидентифицированного значения переменной [Строка:{}]".format(operator[0][2]+1))
                return False
            if not layer.set_variable(name, int(valueF/valueS), iter):
                print("Ошибка: Несовпадение размеров массивов [Строка:{}]".format(operator[0][2]+1))
                return False
    ...
    def calculate_triade(self, triade, layer):
        ...
        if triade['type'] == 'iterator':
            name, iter = self.get_iter(triade, layer)
        ...
        if triade['type'] == 'plus':
            ...
            if type(first) is list or type(second) is list:
                raise Exception("Использование массива в качестве операнда невозможно")
            return first + second
    ...
    def do_for_operator(self, head, sublayer):
        ...
        while self.calculate_triade(head2, sublayer) != 0 and cont:
            for operator in sublayer.operators:
                if operator['type'] == 'DEFAULT':
                    if operator['head'][0][0] == BREAK:
                        cont = False
                        break
                    if not self.do_default_operator(operator['head'], sublayer):
                        return False
                elif operator['type'] == 'FOR':
                    if not self.do_for_operator(operator['head'], operator['sublayer']):
                        return False
            if len(head3)>1:
                if not self.do_default_operator(head3, sublayer):
                    return False
        return True
