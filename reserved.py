...
class Layer:
    ...
    def parse(self):
        ...
        for o in self.operators:
            if 'sublayer' in o.keys():
                o['sublayer'].parse()
    # вот тут подумать че сделать
    def calculate_type(self, type_1, type_2):
        if type_1 == 'int' and type_2 == 'lint':
            return 'lint'
        # if type_1 == 'lint' and type_2 == 'int':
        #     return 'lint'
        return type_2
    ...
    def get_shape(self, arr):
        if type(arr) is int or arr is None:
            return []
        shape = self.get_shape(arr[0])
        shape.append(len(arr))
        return shape
    # тоже что-то новое
    def change_value(self, old_value, iter, value):
        if len(iter) > 0:
            new_value = self.change_value(old_value[iter[0]], iter[1:], value)
            if self.get_shape(old_value[iter[0]]) == self.get_shape(new_value):
                old_value[iter[0]] = new_value
                return old_value
            else:
                return None
        else:
    ...
    def set_variable(self, name, value, iter=None):
        for i in range(len(self.variables)):
            if self.variables[i]['name'] == name:
                if iter is None:
                    if self.get_shape(self.variables[i]['value']) == self.get_shape(value):
                        self.variables[i]['value'] = value
                    else:
                        return False
                else:
                    new_value = self.change_value(self.variables[i]['value'], iter, value)
                    if new_value is None:
                        return False
                    else:
                        if self.get_shape(self.variables[i]['value']) == self.get_shape(new_value):
                            self.variables[i]['value'] = new_value
                        else:
                            return False
                return True
        if self.parent is not None:
                ...
                if is_var_description:
                    ...
                    if is_func_description:
                        ...
                        if lex_arr[0][0] == END:
                            ...
                            if self.parent is None:
                                return False, self.make_err('описание функции, переменной или типа', lex_arr[0]) + '\n' + err_3
                            else:
                    ...
                    if is_A1:
                        if err == '':
                            if new_lex_arr[0][0] == SQUARE_BRACE_CLOSE:
                                if self.accept_warnings and A1_type != 'int' and A1_type != 'lint':
                                    self.add_warning('Ожидалось число или переменная типа int или lint, встречено {}'
                    ...
                    if is_A2_1:
                        if err_1 == '':
                            if self.accept_warnings and A2_type != A2_1_type and \
                                    ((A2_type != 'int' and A2_type != 'lint') or (A2_1_type != 'int' and A2_1_type != 'lint')):
                    ...
                    if is_A3_1:
                        if err_1 == '':
                            if self.accept_warnings and A3_type != A3_1_type  and \
                                    ((A3_type != 'int' and A3_type != 'lint') or (A3_1_type != 'int' and A3_1_type != 'lint')):
                                return True, 'Недопустимая операция {} для типов {} и {} [{}:{}]'\
    ...
    def A6(self, lex_arr):
        if lex_arr[0][0] == TYPE_INT or lex_arr[0][0] == TYPE_SINT:
            if lex_arr[0][0] == TYPE_INT:
                ...
                return True, '', lex_arr[:1], lex_arr[1:], 'lint', 'const'
